...
class IvaSeniatController extends GetxController {
  //var resultados = <RetencionResponse>[].obs;
  ...
  Future<void> cargarIvaSeniat(DateTime desde, DateTime hasta) async {
    cargando(true);
    try {

      final fileName = 'iva_seniat_${DateFormat('yyyyMMdd').format(DateTime.now())}.xlsx';
      await ServiceCuentaUnica.downloadExcelReport(
        url: 'api/query/iva_seniat/excel',
  ...
  Future<void> cargarIvaSeniat2(DateTime desde, DateTime hasta) async {
    cargando(true);
    try {
      final queryParams = {
      ...
      jsonDataAlmacenado.clear();
      final jsonData = await ServiceCuentaUnica.post('api/query/iva_seniat', {}, queryParams: queryParams);

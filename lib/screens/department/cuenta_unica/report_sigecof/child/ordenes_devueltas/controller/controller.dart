...
class DevueltasController extends GetxController{
  ...
  Future<void> cargarDevueltas(DateTime desde, DateTime hasta) async {
    cargando(true);
    try {

      final fileName = 'ordenes_devueltas_${DateFormat('yyyyMMdd').format(DateTime.now())}.xlsx';
      await ServiceCuentaUnica.downloadExcelReport(
        url: 'api/query/ordenes_devueltas/excel',
  ...
  Future<void> cargarDevueltas2(DateTime desde, DateTime hasta) async {
    cargando(true);
    try {
      final queryParams = {
      ...
      jsonDataAlmacenado.clear();
      final jsonData = await ServiceCuentaUnica.post('api/query/ordenes_devueltas', {}, queryParams: queryParams);

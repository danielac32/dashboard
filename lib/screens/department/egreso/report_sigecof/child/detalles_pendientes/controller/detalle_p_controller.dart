...
import '../model/pendiente_detalle.dart';
//import '../service/service.dart';
...
class EgresoPendienteDetalleController extends GetxController {
  var filtro = ''.obs;
  var datos = <DetallePendiente>[].obs;
  var resultados = <DetallePendiente>[].obs;
  var cargando = false.obs;
  final currentPage = 0.obs;
  final itemsPerPage = 20.obs;
  final paginatedResults = <DetallePendiente>[].obs;
  ...
  Future<void> cargarPendientes(DateTime desde, DateTime hasta) async {
    ...
    try {
      ...
      final jsonData = await ServiceEgreso.post('api/query/detalles-pendientes', {}, queryParams: queryParams);
      jsonDataAlmacenado = jsonData as List<dynamic>;
      final List<DetallePendiente> datosLista = (jsonData as List).cast<Map<String, dynamic>>().map((item) => DetallePendiente.fromJson(item)).toList();
  ...
  Future<void> descargarReporte() async {
    ...
    try {
      ...
      // Nombre del archivo
      final fileName = 'reporte_detalles_pendientes_${DateFormat('yyyyMMdd').format(DateTime.now())}.xlsx';
